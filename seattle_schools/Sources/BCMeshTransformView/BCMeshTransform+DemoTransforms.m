...
+ (instancetype)clothMeshTransformAtPoint:(CGPoint)point boundsSize:(CGSize)boundsSize time:(NSTimeInterval)time
{
//    BCMutableMeshTransform *transform = [BCMutableMeshTransform identityMeshTransformWithNumberOfRows:20 numberOfColumns:50];
    BCMutableMeshTransform *transform = [BCMutableMeshTransform identityMeshTransformWithNumberOfRows:2 * 3 numberOfColumns:5 * 3];
    
    [transform mapVerticesUsingBlock:^BCMeshVertex(BCMeshVertex vertex, NSUInteger vertexIndex) {
//        vertex.to.y = (cos((vertex.to.x) + time) * vertex.to.y) * 0.1f + vertex.to.y;  // very good one - high wave
        vertex.to.y = cos(vertex.to.x * 10 + time) * vertex.to.y * 0.05f + vertex.to.y; // very good one too - low wave
        return vertex;
    }];
